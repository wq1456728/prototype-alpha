...
- 2026-05-17 moved skill learning out of the inventory panel into a dedicated `K` skill-tree panel.
- Added config-driven skill loadout defaults in `data/skills/skill_loadout_defaults.json`.
- Added `SkillLoadoutState` with six default slots: `LMB`, `RMB`, `Q`, `E`, `R`, and `V`.
- Added baseline `light_attack` as a default active action that is not part of the skill tree and starts assigned to `LMB`.
- Bottom-center hotbar now displays the six configured slots during normal play.
- Learned active skills can be assigned to slots; unlearned skills and passive skills are rejected.
- Shield Charge can be unlocked in the skill-tree panel, assigned to `V`, and triggered from that slot.
...
Move skill learning out of the inventory panel and add a modern active-skill loadout surface.

Focus on:

- Remove skill-tree controls from the inventory/equipment panel.
- Press `K` to toggle a dedicated skill-tree panel.
- The skill-tree panel should feel closer to Diablo II: a separate character progression surface, not a bag sub-section.
- Keep the first version functional and readable; final art can come later.
- Show configured skill nodes with name, rank, required level, prerequisite state, unlock cost, and locked/unlocked state.
- Allow spending skill points in the skill-tree panel.
- Add a bottom-center skill loadout bar inspired by modern ARPG skill groups.
- The loadout has 6 slots total: left mouse, right mouse, and four keyboard slots.
- Skill loadout slots should be defined by a config file with sensible defaults.
- Default slot bindings are `LMB`, `RMB`, `Q`, `E`, `R`, and `V`.
- Add a default basic `light_attack` skill that represents the normal left-click attack.
- `light_attack` is a baseline action, not a skill-tree node, and should not appear in the skill tree.
- `light_attack` starts assigned to the first hotbar slot, bound to `LMB`.
- Learned active skills can be assigned to loadout slots.
- Locked or unlearned active skills cannot be assigned.
- Passive skills should not be assignable to active slots.
- Skill slots should be visible during normal play, not only when the skill tree is open.
- Current behavior can stay minimal: Shield Charge is the first learned active skill and can be assigned to one slot.
- Keep this as UI/system plumbing; do not add many new skills yet.

Open Design Notes:

- User may provide custom skill-tree art, slot frames, or icons later.
- Until final art exists, use existing icon-style UI assets or simple framed buttons.
- Keep the loadout data separate from learned skill ranks so future respec/loadout changes are possible.
- Suggested first config path: `data/skills/skill_loadout_defaults.json`.

Acceptance:

- `K` opens and closes a dedicated skill-tree panel.
- Inventory no longer contains the skill-tree controls.
- Player can unlock or rank up a skill from the skill-tree panel.
- Bottom-center UI shows 6 active skill slots loaded from config: `LMB`, `RMB`, `Q`, `E`, `R`, and `V` by default.
- `light_attack` is available by default, is not shown as a skill-tree node, and starts assigned to `LMB`.
- A learned active skill can be assigned to a loadout slot.
- An unlearned or passive skill cannot be assigned to an active slot.
- The assigned Shield Charge skill can be triggered from its configured slot.
...
Turn the sandbox into a short vertical-slice objective sequence using the real item, progression, skill-tree, and skill-loadout skeletons.
...
- Require assigning or using a learned active skill from the skill loadout.
...
- The player experiences combat, item drop, inventory, equipment, XP, skill point, skill unlock, skill-slot assignment, skill use, and completion in one flow.
- Completed task entry includes `Task agent status: done`.
...
- 2026-05-16 re-audit: OK against rewritten Milestone 3; item definitions, ItemInstance normalization, generic equipment slot map, and damage stat path are present.
...
- 2026-05-16 re-audit: OK; mummy drops use config-backed drop table, rarity weights, and rolled damage modifiers through ItemDatabase.
...
- 2026-05-16 re-audit: OK; inventory/equipment UI still covers cursor pickup, bag placement, equip/swap/drop, item details, and attack blocking.
...
- 2026-05-16 re-audit: OK; SkillTreeState uses data definitions, level/point requirements, unlock cost, ranks, and gates Shield Charge behavior.
